...
class Session extends EventEmitter {
  ...
  async _createLR4WebSocket(deviceId, options = {}) {
    ...
    if (!userId) {
      throw new WhiskerTokenException('Missing user ID for WebSocket connection');
    }
      ...
      ws.on('message', (data) => {
        ...
        try {
          ...
          switch (json.type) {
            ...
            case 'data': {
              // ByUser subscription returns all of the user's robots in a single message;
              // dispatch only the one matching this device's serial.
              const robots = json.payload?.data?.litterRobot4StateSubscriptionByUser?.robots;
              const targetSerial = st.serial || serial;
              const update = Array.isArray(robots)
                ? robots.find((r) => String(r.serial) === String(targetSerial))
                : null;
              if (update) {
  ...
  _setupHeartbeat(deviceId, ws, idleMs = 90000, checkEveryMs = 45000) {
    ...
    state.heartbeatTimer = this.homey.setInterval(() => {
      ...
      if (timeSinceLastMessage > idleMs) {
        ...
        // Hard-terminate the dead socket; its 'close' handler schedules the reconnect.
        // (1006 is a reserved code that ws.close() rejects, so terminate directly.)
        try {
          currentWs.terminate();
        } catch (termErr) {
          // Ignore - connection may already be closed
